...
window.CJPlaceJourneyMix=placeJourneyMix;
...
function install(){
 let styleRow=$('style')?$('style').closest('.row'):null;if(!$('testTone')&&styleRow){let box=document.createElement('div');box.className='voice-tools';box.innerHTML='<button class="btn" id="testTone" type="button">Test Tone</button><button class="btn" id="nextTestTone" type="button">Next Chakra</button>';styleRow.insertAdjacentElement('afterend',box);$('testTone').onclick=testTone;$('nextTestTone').onclick=()=>{testIndex=(testIndex+1)%7;if(testTimer){stopTest();setTimeout(testTone,250)}else if(typeof status==='function'){let c=window.C[testIndex];status('Tone test chakra: '+c[0]+' • '+c[2]+' Hz')}}}
 let tv=$('tv');if(tv)tv.addEventListener('input',()=>{if(bowl){let a=ac();try{bowl.master.gain.setTargetAtTime(volume(),a.currentTime,.12)}catch(e){}}});
 let st=$('style');if(st)st.addEventListener('change',()=>{if(testTimer){stopTest();setTimeout(testTone,250)}});
 let bowlCtrl=$('bowlReverb');if(bowlCtrl)bowlCtrl.addEventListener('input',applyHallLive);
}
