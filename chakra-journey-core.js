...
let src="both",ctx,tone,yt=$("player"),ytReady=false,run=false,paused=false,fromPreview=false,token=0,cur=-1,watch,voiceNode,voiceGainNode,voiceWarm,voiceDry,voiceWet,aborter,allVoices=[],idx=Object.fromEntries(C.map(c=>[c[0],0])),speakingFor=-1,triedSrc="",voiceDone=null,voiceBuild=null,ducked=false,pendingSeek=0,pendingChakra=-1,historyQuiet=false,leaving=false;
...
function stopTone(f=.45){if(typeof window.__cjStopTone==="function")return window.__cjStopTone(f);if(!tone||!ctx)return;let t=tone;tone=null;let n=ctx.currentTime;try{t.g.gain.setTargetAtTime(.0001,n,.12)}catch{}t.o.forEach(o=>{try{o.stop(n+f)}catch{}})}
function startTone(c){if(typeof window.__cjStartTone==="function")return window.__cjStartTone(c);if(!useTone())return;audio();stopTone();let g=ctx.createGain(),o=[],n=ctx.currentTime,style=$("style").value,l=style==="pure"?[[1,.8]]:style==="tibetan"?[[1,.4],[2.02,.25],[2.98,.14],[4.18,.08]]:style==="warm"?[[.5,.16],[1,.45],[1.5,.14],[2,.1]]:[[1,.48],[2,.23],[3,.12],[4,.06]];g.gain.setValueAtTime(.0001,n);g.connect(ctx.destination);l.forEach(([m,v])=>{let x=ctx.createOscillator(),q=ctx.createGain();x.frequency.value=c[2]*m;q.gain.value=v;x.connect(q);q.connect(g);x.start();o.push(x)});g.gain.linearRampToValueAtTime(+$("tv").value,n+1);tone={g,o}}
...
function applyColorLive(){
  ...
  if(typeof window.__cjApplyHall==="function")window.__cjApplyHall();
}
...
function placeMix(on){if(typeof window.CJPlaceJourneyMix==="function")window.CJPlaceJourneyMix(!!on)}
function finish(t){if(t!==token)return;if(window.chakraAuto&&typeof window.chakraAuto.sessionEnd==="function")window.chakraAuto.sessionEnd("completed");run=false;paused=false;speakingFor=-1;document.body.classList.remove("journey-on");placeMix(false);stopVoice();stopTone();if(yt)try{yt.pause()}catch(e){}$("play").textContent="Start Journey";status("Journey complete");popJourneyHistory();writeSession({active:false,paused:false,chakra:-1,videoTime:0,mode:$("mode")?$("mode").value:"",src:src})}
function startYT(){if(!useYT()){pendingSeek=0;return true}if(!ytReady){status("Video is still loading");return false}try{yt.setAttribute("playsinline","");yt.setAttribute("webkit-playsinline","");yt.playsInline=true;yt.currentTime=Math.max(0,pendingSeek||0);pendingSeek=0;fadeYT(ytVol());let p=yt.play();if(p&&p.catch)p.catch(()=>{yt.muted=true;yt.play().then(()=>{yt.muted=false;fadeYT(ytVol())}).catch(()=>status("Tap the video play button"))});return true}catch(e){return false}}
function pushJourneyHistory(){if(history.state&&history.state.cj==="journey")return;try{history.pushState({cj:"journey"},"")}catch(e){}}
function popJourneyHistory(){if(historyQuiet)return;if(!(history.state&&history.state.cj==="journey"))return;historyQuiet=true;try{history.back()}catch(e){historyQuiet=false}}
function fadeLiveAudio(){try{let a=ctx,now=a?a.currentTime:0;if(a&&tone&&tone.g){tone.g.gain.cancelScheduledValues(now);tone.g.gain.setTargetAtTime(.0001,now,.12)}if(a&&voiceGainNode)voiceGainNode.gain.setTargetAtTime(.0001,now,.12);if(typeof window.__cjStopTone==="function")window.__cjStopTone(1.2);if(yt)try{yt.volume=Math.max(0,(yt.volume||0)*.15)}catch(e){}}catch(e){}}
function start(){hideVoiceGap();if(window.chakraAuto&&typeof window.chakraAuto.sessionStart==="function")window.chakraAuto.sessionStart();unlockMedia();if(run&&paused)return togglePause();if(run)stop({keepHistory:true});stopVoice();unlockVoiceAudio();decks={};speakingFor=-1;token++;let t=token;run=true;paused=false;cur=-1;document.body.classList.add("journey-on");placeMix(true);$("play").textContent="Running";if(!startYT()){run=false;document.body.classList.remove("journey-on","immersive","exhale");placeMix(false);$("play").textContent="Start Journey";return}let m=$("mode").value;if(m==="timestamps"){if(!useYT()){status("Timestamp Sync requires the video");run=false;document.body.classList.remove("journey-on","immersive","exhale");placeMix(false);$("play").textContent="Start Journey";return}timestampRun(t)}else sequence(t,m==="timed");pushJourneyHistory();writeSession()}
function togglePause(){if(!run)return;paused=!paused;if(useYT()&&ytReady)paused?yt.pause():yt.play().catch(()=>{});if(paused){try{if(ctx&&ctx.state==="running")ctx.suspend()}catch(e){}status("Journey paused");$("pause").textContent="Resume"}else{try{if(ctx&&ctx.state==="suspended")ctx.resume()}catch(e){}unlockVoiceAudio();$("pause").textContent="Pause";status("Journey resumed")}writeSession()}
function stop(opts){let keep=opts&&opts.keepHistory;let shot=journeySnapshot();if(run||paused)shot.active=true;if(run&&window.chakraAuto&&typeof window.chakraAuto.sessionEnd==="function")window.chakraAuto.sessionEnd("stopped");token++;run=false;paused=false;speakingFor=-1;document.body.classList.remove("journey-on");placeMix(false);clearInterval(watch);stopVoice();stopTone();if(ytReady){try{yt.pause();yt.currentTime=0}catch(e){}}current(-1,"Journey stopped");$("play").textContent="Start Journey";$("pause").textContent="Pause";status();if(!keep)popJourneyHistory();if(!opts||!opts.skipSession)writeSession(shot)}
function returnToMain(fromPop){if(leaving)return;leaving=true;let shot=journeySnapshot();if(run||paused)shot.active=true;fadeLiveAudio();setTimeout(()=>{if(run||paused)stop({keepHistory:true,skipSession:true});else{document.body.classList.remove("immersive","exhale","journey-on");placeMix(false)}writeSession(shot);if(!fromPop)popJourneyHistory();leaving=false;renderSessionPanel();status("Back on the main screen");window.scrollTo({top:0,behavior:"smooth"})},280)}
...
function save(){let ids=["yv","dv","mode","dur","count","gap","between","voice","key","voicePreset","model","speed","stability","similarity","voiceStyle","warmthFreq","reverbMix","bowlReverb","vv","style","tv"];ids.forEach(id=>{if($(id))localStorage.setItem("cj_"+id,$(id).value)});localStorage.setItem("cj_src",src);if($("speakerBoost"))localStorage.setItem("cj_speakerBoost",$("speakerBoost").checked?"1":"0");if($("autoModel"))localStorage.setItem("cj_autoModel",$("autoModel").checked?"1":"0");if($("goveeOn"))localStorage.setItem("cj_goveeOn",$("goveeOn").checked?"1":"0");C.forEach((_,i)=>{let el=document.querySelector('[data-ts="'+i+'"]');if(el)localStorage.setItem("cj_ts"+i,el.value)})}
function load(){let ids=["yv","dv","mode","dur","count","gap","between","voice","key","voicePreset","model","speed","stability","similarity","voiceStyle","warmthFreq","reverbMix","bowlReverb","vv","style","tv"];ids.forEach(id=>{let v=localStorage.getItem("cj_"+id);if(v!==null&&$(id))$(id).value=v});let boost=localStorage.getItem("cj_speakerBoost");if(boost!==null&&$("speakerBoost"))$("speakerBoost").checked=boost==="1";let auto=localStorage.getItem("cj_autoModel");if(auto!==null&&$("autoModel"))$("autoModel").checked=auto==="1";let govee=localStorage.getItem("cj_goveeOn");if(govee!==null&&$("goveeOn"))$("goveeOn").checked=govee==="1";src=localStorage.getItem("cj_src")||"both";C.forEach((_,i)=>{let v=localStorage.getItem("cj_ts"+i);let el=document.querySelector('[data-ts="'+i+'"]');if(v!==null&&el)el.value=v});if(localStorage.getItem("cj_pitchfix")!=="1"){applyAnalyzed();localStorage.setItem("cj_pitchfix","1")}ui()}
const SESSION_KEY="cj_session_v1";
function sessionSettings(){let ids=["yv","dv","mode","dur","count","gap","between","voice","key","voicePreset","model","speed","stability","similarity","voiceStyle","warmthFreq","reverbMix","bowlReverb","vv","style","tv","goveeBright","goveeKey"];let settings={};ids.forEach(id=>{if($(id))settings[id]=$(id).value});settings.speakerBoost=!!$("speakerBoost")?.checked;settings.autoModel=!!$("autoModel")?.checked;settings.goveeOn=!!$("goveeOn")?.checked;settings.src=src;settings.timestamps=C.map((_,i)=>{let el=document.querySelector('[data-ts="'+i+'"]');return el?el.value:""});return settings}
function journeySnapshot(){return {active:!!run,paused:!!paused,chakra:cur,videoTime:yt&&Number.isFinite(yt.currentTime)?yt.currentTime:0,mode:$("mode")?$("mode").value:"",src:src}}
function writeSession(journeyOverride){let journey=journeyOverride||journeySnapshot();if(!journeyOverride&&!run&&!paused){let prev=readSession();if(prev&&prev.journey&&prev.journey.active)journey=prev.journey}let blob={v:"0.33",savedAt:new Date().toISOString(),settings:sessionSettings(),journey:journey};try{localStorage.setItem(SESSION_KEY,JSON.stringify(blob))}catch(e){}renderSessionPanel();return blob}
function readSession(){try{return JSON.parse(localStorage.getItem(SESSION_KEY)||"null")}catch(e){return null}}
function applySessionSettings(s){if(!s)return;["yv","dv","mode","dur","count","gap","between","voice","key","voicePreset","model","speed","stability","similarity","voiceStyle","warmthFreq","reverbMix","bowlReverb","vv","style","tv","goveeBright","goveeKey"].forEach(id=>{if(s[id]!=null&&$(id))$(id).value=s[id]});if($("speakerBoost")&&s.speakerBoost!=null)$("speakerBoost").checked=!!s.speakerBoost;if($("autoModel")&&s.autoModel!=null)$("autoModel").checked=!!s.autoModel;if($("goveeOn")&&s.goveeOn!=null)$("goveeOn").checked=!!s.goveeOn;if(s.src)src=s.src;(s.timestamps||[]).forEach((v,i)=>{let el=document.querySelector('[data-ts="'+i+'"]');if(el&&v!=null)el.value=v});ui();try{applyColorLive()}catch(e){}}
function renderSessionPanel(){let el=$("sessionInfo"),blob=readSession();if(!el)return;if(!blob){el.textContent="No saved session on this device yet.";return}let when=blob.savedAt?new Date(blob.savedAt).toLocaleString():"earlier";let j=blob.journey||{},place=j.active&&j.chakra>=0&&C[j.chakra]?C[j.chakra][0]:"setup";el.textContent="Saved "+when+" • "+place+(j.active?" • in progress, ready to resume":" • settings only")+"."}
function resumeSession(){let blob=readSession();if(!blob){status("No saved session");return}applySessionSettings(blob.settings||{});save();let j=blob.journey||{};if(j.active){pendingChakra=-1;pendingSeek=0;if((blob.settings&&blob.settings.src)||src){if(((blob.settings&&blob.settings.mode)||$("mode").value)!=="timestamps")pendingChakra=j.chakra>=0?j.chakra:0;else pendingSeek=j.videoTime||0}if(run)stop({keepHistory:true,skipSession:true});start();status("Session restored")}else status("Session settings restored");renderSessionPanel()}
function ui(){document.querySelectorAll("#tabs button").forEach(b=>b.classList.toggle("on",b.dataset.s===src));if($("ytBox"))$("ytBox").classList.toggle("hide",!useYT());if($("yvv"))$("yvv").textContent=$("yv").value+"%";if($("dvv"))$("dvv").textContent=$("dv").value+"%";if($("vvv"))$("vvv").textContent=Math.round(+$("vv").value*100)+"%";if($("tvv"))$("tvv").textContent=Math.round(+$("tv").value*100)+"%";if($("speedv"))$("speedv").textContent=(+$("speed").value).toFixed(2)+"x";if($("stabilityv"))$("stabilityv").textContent=Math.round(+$("stability").value*100)+"%";if($("similarityv"))$("similarityv").textContent=Math.round(+$("similarity").value*100)+"%";if($("voiceStylev"))$("voiceStylev").textContent=Math.round(+$("voiceStyle").value*100)+"%";if($("warmthFreqv"))$("warmthFreqv").textContent=$("warmthFreq").value+" Hz";if($("reverbMixv"))$("reverbMixv").textContent=(+$("reverbMix").value*100).toFixed(1)+"%";if($("bowlReverbv")&&$("bowlReverb"))$("bowlReverbv").textContent=Math.round(+$("bowlReverb").value*100)+"%";let m=$("mode").value;if($("ts"))$("ts").classList.toggle("show",m==="timestamps");if($("durrow"))$("durrow").style.display=m==="timed"?"flex":"none"}
...
["vv","tv","warmthFreq","reverbMix","bowlReverb"].forEach(id=>{if($(id))$(id).addEventListener("input",()=>{ui();save();applyColorLive()})});
...
if($("backMain"))$("backMain").onclick=()=>returnToMain(false);
if($("sessionSave"))$("sessionSave").onclick=()=>{writeSession();status("Session saved on this device")};
if($("sessionResume"))$("sessionResume").onclick=()=>resumeSession();
window.addEventListener("popstate",()=>{if(historyQuiet){historyQuiet=false;return}if(run||paused||document.body.classList.contains("immersive")||document.body.classList.contains("journey-on"))returnToMain(true)});
window.addEventListener("pagehide",()=>{try{if(!leaving)writeSession()}catch(e){}});
window.CJSaveSession=writeSession;window.CJResumeSession=resumeSession;window.CJReturnToMain=returnToMain;
...
loadVideoSrc(VIDEO_FILES[0]);if(yt.readyState>=1)ytReady=true;load();renderSessionPanel();setTimeout(updateVoiceAccount,400);
