...
function start(){hideVoiceGap();if(window.chakraAuto&&typeof window.chakraAuto.sessionStart==="function")window.chakraAuto.sessionStart();unlockMedia();if(run&&paused)return togglePause();if(run)stop({keepHistory:true});stopVoice();unlockVoiceAudio();decks={};speakingFor=-1;token++;let t=token;run=true;paused=false;cur=-1;document.body.classList.add("journey-on");$("play").textContent="Running";if(!startYT()){run=false;document.body.classList.remove("journey-on","immersive","exhale");$("play").textContent="Start Journey";return}let m=$("mode").value;if(m==="timestamps"){if(!useYT()){status("Timestamp Sync requires the video");run=false;document.body.classList.remove("journey-on","immersive","exhale");$("play").textContent="Start Journey";return}timestampRun(t)}else sequence(t,m==="timed");pushJourneyHistory();writeSession()}
...
function stop(opts){let keep=opts&&opts.keepHistory;if(run&&window.chakraAuto&&typeof window.chakraAuto.sessionEnd==="function")window.chakraAuto.sessionEnd("stopped");token++;run=false;paused=false;speakingFor=-1;document.body.classList.remove("journey-on");clearInterval(watch);stopVoice();stopTone();if(ytReady){try{yt.pause();yt.currentTime=0}catch(e){}}current(-1,"Journey stopped");$("play").textContent="Start Journey";$("pause").textContent="Pause";status();if(!keep)popJourneyHistory();if(!opts||!opts.skipSession)writeSession()}
function returnToMain(fromPop){if(leaving)return;leaving=true;let shot=journeySnapshot();if(run||paused)shot.active=true;fadeLiveAudio();setTimeout(()=>{if(run||paused)stop({keepHistory:true,skipSession:true});else document.body.classList.remove("immersive","exhale","journey-on");writeSession(shot);if(!fromPop)popJourneyHistory();leaving=false;renderSessionPanel();status("Back on the main screen");window.scrollTo({top:0,behavior:"smooth"})},280)}
...
function writeSession(journeyOverride){let blob={v:"0.32",savedAt:new Date().toISOString(),settings:sessionSettings(),journey:journeyOverride||journeySnapshot()};try{localStorage.setItem(SESSION_KEY,JSON.stringify(blob))}catch(e){}renderSessionPanel();return blob}
function readSession(){try{return JSON.parse(localStorage.getItem(SESSION_KEY)||"null")}catch(e){return null}}
function applySessionSettings(s){if(!s)return;["yv","dv","mode","dur","count","gap","between","voice","key","voicePreset","model","speed","stability","similarity","voiceStyle","warmthFreq","reverbMix","vv","style","tv","goveeBright","goveeKey"].forEach(id=>{if(s[id]!=null&&$(id))$(id).value=s[id]});if($("speakerBoost")&&s.speakerBoost!=null)$("speakerBoost").checked=!!s.speakerBoost;if($("autoModel")&&s.autoModel!=null)$("autoModel").checked=!!s.autoModel;if($("hallEffect")&&s.hallEffect!=null)$("hallEffect").checked=!!s.hallEffect;if($("goveeOn")&&s.goveeOn!=null)$("goveeOn").checked=!!s.goveeOn;if(s.src)src=s.src;(s.timestamps||[]).forEach((v,i)=>{let el=document.querySelector('[data-ts="'+i+'"]');if(el&&v!=null)el.value=v});ui();try{applyColorLive()}catch(e){}}
...
if($("hallEffect"))$("hallEffect").addEventListener("change",()=>{save();applyColorLive()});
